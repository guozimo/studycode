...
function creatMPagesRecords(
	type, pageNumber,showInput,place,TextFont,TextSize,TextColor,lines,Command,SetInitValue,isPage,isDisp)
{
	$(document).ready(function(){

		var stt=lines.split("|");
		var sp=SetInitValue.split("|")[3];
		if(sp==stt[0]){
			document.getElementById("choose").selectedIndex =1;

		}else if(sp==stt[1]){
			document.getElementById("choose").selectedIndex =2;


		}else if(sp==stt[2]){
			document.getElementById("choose").selectedIndex =3;


		}else if(sp==stt[3]){

			document.getElementById("choose").selectedIndex =4;
		}else if(sp==stt[4]){
			document.getElementById("choose").selectedIndex =5;
		}else {

			//SubmitInputPage(Command,lines);
...
//type缺省
	...
	MPagesRecordsHTML+="<input type='text' style='display:none' name='lines' id='lines' class='cur' value="+SetInitValue.split("|")[3]+">";
    //这条是为了把动态变化的值传进去用的
	//当前页 总页数 总条数
	//命令为空则无翻页按钮
	...
	if(Command.length>0||Command!='')
	{
		...
		if(disp==1) {
			...
			if(ShowInput == "1"){
				...
				MPagesRecordsHTML+="<a href='javascript:;' onclick='SubmitInputPage(\""+Command+"\"" +",\""+lines+"\");' style='color:#416aa3; width:25px; height:16px; font-size:12px; line-height:16px; pading:0px; margin:0px; margin-left:3px; text-align:center;'>Go</a>";
	...
	if(lines){
		...
		str+="<select  onchange='SubmitInputPage(\""+Command+"\"" +",\""+lines+"\");' name="+'choose'+" id="+'choose'+" style='width:60px;height:24px;position:absolute;top:2px;left:0;'>";
		...
		for(var i = 0; i < str1.length;i ++){
			...
			//MPagesRecordsHTML+="<input type='text' style='display:block' name='lines' id='lines' class='cur' value="+lines.split("|")[3]+">";
		}
		str+="</select>";
		str+="</div>";
...
function SubmitInputPage(CMD,lines){
	...
	var _Sizeopt5 = eval(opp[4]);

try{
	if($("#choose").get(0).selectedIndex&&reg.test(_inputPage.value)){
		eval(document.form.currentPage).value = parseInt(_inputPage.value);
	}

	//
	if($("#choose").get(0).selectedIndex==1){
		$("#lines").val(_Sizeopt1.value);
		//   eval(document.form.lines).value = parseInt(_Sizeopt1.value);

	}else if($("#choose").get(0).selectedIndex==2){
		$("#lines").val(_Sizeopt2.value);
	}else if($("#choose").get(0).selectedIndex==3){
		$("#lines").val(_Sizeopt3.value);
	}else if($("#choose").get(0).selectedIndex==4){
		$("#lines").val(_Sizeopt4.value);
	}else if($("#choose").get(0).selectedIndex==5){
		$("#lines").val(_Sizeopt5.value);
	}else{
...
//设定第row行第col列的初始值
{
	...
	if(typeof(MainTableRowNum)!="undefined")
		for(i=1;i<=MainTableRowNum;i++)
		{
			...
			//console.log(MainListInitValue[i]);   输出不出来
		}
// //转义某些字符
...
//指定第number列的名称(name)，显示列名(text)和宽度(width)
{
	...
	$("document").ready(function () {
		...
		for (var k = 1; k < MainListInitValue.length; k++) {
			...
			eval("var"+" "+"a"+number+""+"="+"cur["+number+"]");
			eval(("ary"+number)).push(eval(("a"+number)));
		}
		colsCompare=getTextBeforeComma(ColsCompare);
		if ((len-1) % 2 == 0 && colsCompare) {
			for (var t = 0; t <(len-2); t += 2) {
				var C = eval(("ary" + number) + "[" + (t) + "]");
				var D = eval(("ary" + number) + "[" + (t + 1) + "]");
				if (C != "undefined" && D != "undefined") {
					if (C == D) {
						...
						str1=reg1.exec(colsCompare);
						str2=reg2.exec(colsCompare);
						if(reg1.test(colsCompare)){
							var color1=str1[1];
							//和平台的结构略有不同 平台那边是nobr 样例这边是font  走block=#XXX
							$("#tablecontent_" +number+ "_" + (t+1) + "").children("nobr").css("color",color1);
							$("#tablecontent_" +number+ "_" + (t+2) + "").children("nobr").css("color", color1);
						}else if(reg2.test(colsCompare)){
							var color2=str2[1];
							$("#tablecontent_" +number+ "_" + (t+1) + "").css("background",color2);
							$("#tablecontent_" +number+ "_" + (t + 2) + "").css("background",color2);
						}
					}
				}
	...
	function addClass(ele,strClass){
	 for (var i=0;i<ele.length;i++){
	    ...
	    if(!reg.test(atr.className))
	       atr.className+=" "+strClass;
	 }
	}
	...
	//console.log(MainListColRowSelectForbidCondition[3]);这是第一列的管理员
...
function MainListChangeCheckState(tn,col,row,radio,pv)
{
	...
	if (radio==1)
	{
		...
		if (document.getElementsByName(p)[0].value == "1") {
			...
			//如果没打勾
			//放在一个隐藏域名的input上面了
			//如果这个中的某一列的值不等于下面的这个值 就进行勾选
			//console.log(MainListColRowSelectForbidCondition);

			var n=MainListColRowSelectForbidCondition.length;

			for(var m=0;m<n;m++){

				eval("var"+" "+"forbid"+m+""+"="+"MainListColRowSelectForbidCondition["+(m+3)+"]");
				eval("var"+" "+"ary"+m+""+"="+"[]");
				//console.log(eval("ary"+m));
			}
			...
			for (var k = 1; k < MainListInitValue.length; k++) {
				...
				for(var j=0;j<n;j++){
					eval("var"+" "+"a"+j+""+"="+"cur["+(j+3)+"]");
					eval(("ary"+j)).push(eval(("a"+j)));

				}

			}

			var tureOrFalse=function(){
				for(var i=0;i<n;i++){
					var A=eval(("ary"+i)+"["+(row-1)+"]");
					var B=eval(("forbid"+i));
					if(A&&B){
						if(A==B){
							return true;
...
// HeadHTML1+="</tr>\n";
		...
		//for (k=2;k<=3;k++)
	{
		...
		for (i=MainTableColNum+1;i<=MainListTablCol;i++)
		{
			...
			//console.log(MainListInitValue[j][3]);
			//console.log(MainListInitValue[j+1][3]);
...
function showMainListCommColAndValue(j,i,rowcolor, ColShowConditionColor, InitValue,nobr)
{
	...
	//文字方式
	//console.log(MainListColShowValueStyle);

	if(MainListColShowValueStyle[i]==0)
	{


		//进来一个创建一个input   InitValue 这个value值也是动态创建的  有几条数据 对应几个value
		...
		for(a=0;a<ml;a++)
		{
			if(i==mm[a])
			{
				...
				//console.log(mainListcolclassify);
		...
		{
			//console.log(MainListColshowvalue[3]);
			//列显示值
		...
		{
			//为空的行
			//console.log(MainListColshowvalue[i]);
			//走这条路 MainListColshowvalue[i]为空
